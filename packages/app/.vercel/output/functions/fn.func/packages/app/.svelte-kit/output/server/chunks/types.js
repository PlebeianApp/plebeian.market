...
import { M as META_NAMES, b as META_SCOPES, c as META_DATA_TYPES, P as PAYMENT_DETAILS_METHOD, d as PRODUCT_TYPES, e as PRODUCT_IMAGES_TYPE, A as AUCTION_STATUS, B as BID_STATUS, O as ORDER_STATUS, I as INVOICE_STATUS } from "./constants.js";
...
const dbPath = path.resolve(process.cwd(), "..", "app", "sqlite.db");
const config = {
  schema: "./schema.ts",
  out: "./drizzle",
  dialect: "sqlite",
...
const standardProductColumns = {
  stallId: text("stall_id").notNull().references(() => stalls.id, { onDelete: "cascade", onUpdate: "cascade" }),
...
const metaTypes = sqliteTable("meta_types", {
  name: text("name", { enum: Object.values(META_NAMES) }).primaryKey(),
  description: text("description"),
  scope: text("scope", { enum: Object.values(META_SCOPES) }).notNull(),
  dataType: text("data_type", { enum: Object.values(META_DATA_TYPES) }).notNull()
});
const productMeta = sqliteTable("product_meta", {
  ...
  metaName: text("meta_name").notNull().references(() => metaTypes.name, { onDelete: "cascade", onUpdate: "cascade" }),
  key: text("key"),
...
const appSettingsMeta = sqliteTable("app_meta", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
...
const events = sqliteTable("events", {
  ...standardColumns,
  author: text("author").notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
...
const users = sqliteTable("users", {
  ...
  lastLogin: integer("last_login", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`)
});
...
const stalls = sqliteTable("stalls", {
  ...standardColumns,
...
const paymentDetails = sqliteTable("payment_details", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  stallId: text("stall_id").references(() => stalls.id, { onUpdate: "cascade" }),
  paymentMethod: text("payment_method", { enum: Object.values(PAYMENT_DETAILS_METHOD) }).notNull(),
  paymentDetails: text("payment_details").notNull(),
  isDefault: integer("default", { mode: "boolean" }).notNull().default(false)
});
const shipping = sqliteTable("shipping", {
  ...standardColumns,
  stallId: text("stall_id").references(() => stalls.id, { onUpdate: "cascade" }),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  name: text("name").notNull(),
...
const shippingZones = sqliteTable("shipping_zones", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  shippingId: text("shipping_id").notNull().references(() => shipping.id, { onDelete: "cascade", onUpdate: "cascade" }),
  stallId: text("stall_id").references(() => stalls.id, { onUpdate: "cascade" }),
  regionCode: text("region_code").notNull(),
  countryCode: text("country_code").notNull()
});
const shippingRelations = relations(shipping, ({ many }) => ({
  shippingZones: many(shippingZones)
}));
const shippingZoneRelations = relations(shippingZones, ({ one }) => ({
  shipping: one(shipping, { fields: [shippingZones.shippingId], references: [shipping.id] })
}));
const products = sqliteTable("products", {
  ...standardColumns,
  ...standardProductColumns,
  productType: text("product_type", { enum: Object.values(PRODUCT_TYPES) }).notNull().default("simple"),
  parentId: text("parent_id").references(() => products.id, { onDelete: "cascade", onUpdate: "cascade" }),
  price: numeric("price").notNull()
});
const productImages = sqliteTable(
  "product_images",
  {
    ...
    imageType: text("image_type", { enum: Object.values(PRODUCT_IMAGES_TYPE) }).notNull().default("gallery"),
...
const categories = sqliteTable("categories", {
  ...
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  description: text("description").notNull(),
  parentId: text("parent_id").references(() => categories.id, { onDelete: "cascade", onUpdate: "cascade" })
});
...
const auctions = sqliteTable("auctions", {
  ...standardColumns,
  ...standardProductColumns,
  startingBidAmount: numeric("starting_bid_amount").notNull(),
  status: text("status", { enum: Object.values(AUCTION_STATUS) }).notNull(),
  startDate: integer("start_date", { mode: "timestamp" }).default(sql`(unixepoch())`),
  endDate: integer("end_date", { mode: "timestamp" })
});
const bids = sqliteTable("bids", {
  ...standardColumns,
  auctionId: text("auction_id").notNull().references(() => auctions.id, { onDelete: "cascade", onUpdate: "cascade" }),
  userId: text("user_id").notNull().references(() => users.id),
  bidAmount: numeric("bid_amount").notNull(),
  bidStatus: text("bid_status", { enum: Object.values(BID_STATUS) }).notNull().default("pending")
});
const orders = sqliteTable("orders", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`),
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`),
  sellerUserId: text("seller_user_id").notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  buyerUserId: text("buyer_user_id").notNull().references(() => users.id),
  status: text("status", { enum: Object.values(ORDER_STATUS) }).notNull().default("pending"),
  shippingId: text("shipping_id").notNull().references(() => shipping.id),
  stallId: text("stall_id").notNull().references(() => stalls.id, { onDelete: "cascade", onUpdate: "cascade" }),
...
const orderItems = sqliteTable(
  "order_items",
  {
    orderId: text("order_id").notNull().references(() => orders.id, { onDelete: "cascade", onUpdate: "cascade" }),
...
const invoices = sqliteTable("invoices", {
  ...
  invoiceStatus: text("invoice_status", { enum: Object.values(INVOICE_STATUS) }).notNull().default("pending"),
  paymentDetails: text("payment_details_id").notNull().references(() => paymentDetails.id)
});
...
export {
  ...
  userMeta as f,
  products as g,
  getTableColumns as h,
  productMeta as i,
  categories as j,
  productCategories as k,
  productImages as l,
  stalls as m,
  shipping as n,
  shippingZones as o,
