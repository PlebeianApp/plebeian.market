...
const login = async (page: Page) => {
  ...
  await page.fill(
    "#signInSk",
    "nsec159fuyawnrq4s2gxjdjuhusvz5s5cfxamu8kfkmkunrz90ydtwzfqsl6g37",
  );
...
describe(
  ...
  async () => {
    ...
    beforeAll(async () => {
      ...
      await page.goto(
        `http://${process.env.APP_HOST}:${process.env.APP_PORT}/settings`,
      );
    ...
    it("should navigate to user settings and submit the form", async () => {
      await page.goto(
        `http://${process.env.APP_HOST}:${process.env.APP_PORT}/settings/account/profile`,
      );
      await login(page);
