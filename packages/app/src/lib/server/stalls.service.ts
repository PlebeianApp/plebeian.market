...
import { PaymentDetail, Shipping, Stall } from "@plebeian/database";
import {
	and,
	db,
	eq,
...
const getZonesToInsert = (
  shippingResult: Shipping,
  regions?: string[],
  countries?: string[],
) => {
  if (regions?.length && countries?.length) {
    return regions.flatMap((region: string) =>
      (countries ?? []).map((country: string) =>
        createZone(country, region, shippingResult),
      ),
    );
  } else if (regions?.length) {
    return regions.map((region: string) =>
      createZone(null, region, shippingResult),
    );
  } else if (countries?.length) {
    const z = (countries ?? []).map((country: string) =>
      createZone(country, null, shippingResult),
    );
    return z;
  } else {
    return [];
...
const resolveStalls = async (stall: Stall): Promise<RichStall> => {
  ...
  const paymentMethods = await db
    .select()
    .from(paymentDetails)
    .where(eq(paymentDetails.stallId, stall.id))
    .execute();
...
export const getAllStalls = async (
  filter: StallsFilter = stallsFilterSchema.parse({}),
) => {
  const orderBy = {
...
const stallsByCatNamePrepared = db
  ...
  .where(
    and(
      eq(eventTags.tagValue, sql.placeholder("catName")),
...
export const getStallsByCatName = async (
  catName: string,
): Promise<RichStall[]> => {
...
export const getStallsByUserId = async (
  userId: string,
): Promise<RichStall[]> => {
  const stallsResult = await db
    .select()
    .from(stalls)
    .where(eq(stalls.userId, userId))
    .execute();
      ...
      promises.push(
        db
          ...
          .then(([shippingResult]) => {
            ...
            const zonesToInsert = getZonesToInsert(
              shippingResult,
              method.regions,
              method.countries,
            );
...
export const updateStall = async (
  ...
  try {
    ...
    const {
      ...
      .partial()
      .safeParse({ id: stallId, ...stallEventContent });
    if (!success) {
    ...
    const [image] = await db
      ...
      .from(eventTags)
      .where(
        and(eq(eventTags.eventId, stallId), eq(eventTags.tagName, "image")),
      )
      .execute();
    ...
    if (parsedStall.shipping && parsedStall.shipping.length > 0) {
      try {
        await db.transaction(async (tx) => {
          ...
          const newShippingMethodsMap = new Map(
            parsedStall.shipping?.map((method) => [method.id, method]),
          );
          const existingShippingMethodsMap = new Map(
            existingShippingMethods.map((method) => [method.id, method]),
          );

          const methodsToInsert =
            parsedStall.shipping?.filter(
              (method) => !existingShippingMethodsMap.has(method.id),
            ) ?? [];
          const methodsToUpdate =
            parsedStall.shipping?.filter((method) =>
              existingShippingMethodsMap.has(method.id),
            ) ?? [];
          const methodsToDelete = existingShippingMethods.filter(
            (method) => !newShippingMethodsMap.has(method.id),
          );
          ...
          const shippingMethodIds =
            parsedStall.shipping?.map((method) => method.id) ?? [];
          ...
          const existingZones = await tx
            ...
            .where(
              and(
                ...
                inArray(shippingZones.shippingId, shippingMethodIds),
              ),
            );
