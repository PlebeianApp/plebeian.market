...
export async function btcToCurrency(currency: string) {
  try {
    const { result } = await ofetch(
      `https://api.yadio.io/convert/1/btc/${currency}`,
    );
    return result;
  } catch (error) {
...
export function getEventCoordinates(
  event: NostrEvent | VerifiedEvent | NDKEvent,
...
export function customTagValue(
  eventTags: NDKTag[],
  key: string,
  thirdValue?: string,
): string[] {
  const values = eventTags
    .filter(
      ([k, v, t]) =>
        k === key && (thirdValue === undefined || t === thirdValue),
    )
...
export async function shouldRegister(
  ...
  if (allowRegister == undefined)
    allowRegister = get(page).data.appSettings.allowRegister;
...
export const calculateGeohashAccuracy = (
...
export function mergeWithExisting<T>(
  existing: T[],
  newItems: T[],
  key: keyof T,
) {
  ...
  return [
    ...existing,
    ...newItems.filter((item) => !existingSet.has(item[key])),
  ];
}
