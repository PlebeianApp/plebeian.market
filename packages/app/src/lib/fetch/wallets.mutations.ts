...
export const persistWalletMutation = createMutation(
	{
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({ queryKey: walletKeys.details($ndkStore.activeUser?.pubkey) })
		},
	},
	queryClient,
)
...
export const updateOnChainIndexMutation = createMutation(
	{
		...
		onSuccess: () => {
			const $ndkStore = get(ndkStore)
			if (!$ndkStore.activeUser?.pubkey) return
			queryClient.invalidateQueries({ queryKey: walletKeys.onChain($ndkStore.activeUser?.pubkey, '') })
		},
	},
	queryClient,
)
