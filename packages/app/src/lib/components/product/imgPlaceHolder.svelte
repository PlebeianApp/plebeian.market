<script lang="ts">
	...
	if (imageType == 'main') {
		...
		width = 80
		height = 80
	} else if (imageType == 'manual') {
		width = width
		height = height
	}
</script>

<svg {width} {height} xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
	<defs>
		<style type="text/css">
			#holder text {
				fill: #000000;
	...
	<g id="holder">
		...
		<g>
			<text text-anchor="middle" x="50%" y="50%" dy=".3em">{width} x {height}</text>
		</g>
	</g>
</svg>
