<script lang="ts">
	...
	import { defaulRelaysUrls, ndk } from '$lib/stores/ndk'
	...
	<div class="mx-auto w-full max-w-2xl flex flex-col gap-2">
		...
		<Tabs.Root value="stalls" class="w-full flex items-start gap-6">
			...
			<Tabs.Content value="network" class="flex flex-col w-full gap-6">
				...
				<Alert.Root class="bg-[var(--neo-blue)]">
					<Alert.Description
						>Your products are published to these relays (servers). You cannot modify this setting for now. Please let us know if you’d like
						to add a relay.</Alert.Description
					>
				</Alert.Root>
