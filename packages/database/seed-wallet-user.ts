...
const WALLETED_USER_NPUB =
  "npub1u2d97q0eeppas8t2t9lvqdxj7ltgv3pgp3wqe5w5w2e78yv3tk6q30rtnr";
const WALLETED_USER_HEX =
  "e29a5f01f9c843d81d6a597ec034d2f7d68644280c5c0cd1d472b3e391915db4";
const XPUB =
...
const main = async () => {
  ...
  const userMeta = [V4V_DEFAULT_RECIPIENTS[1], V4V_DEFAULT_RECIPIENTS[2]].map(
    (value) => {
  ...
  const uniqueShippingZonesData = Array.from(
    new Map(
      shippingZonesData
        .flat()
        .map((zone) => [
          `${zone.shippingId}-${zone.regionCode}-${zone.countryCode}`,
          zone,
        ]),
    ).values(),
  );
